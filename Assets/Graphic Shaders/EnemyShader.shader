...
Shader "Custom/Enemy Shader"
{
    ...
    SubShader
    {
        ...
        shared bool ShouldDraw(float depth, float4 screenPosition)
        {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f input) : SV_Target
            {
				...
				float worldLightFactor = max(0, dot(input.worldNormal, _WorldSpaceLightPos0));
				float4 lightColor = _LightColor0 * worldLightFactor;
				color.rgb = color.rgb * lightColor.rgb * tintColor.rgb;
                color.rgb = color.rgb * (1 - flashColor.a) + flashColor.rgb * flashColor.a;
                return color;
            };
			ENDCG
        }
